...
int tty_fd, flags;
unsigned char c='D';
/* Lecture du le kit de développement kineis */
int r_kineis(char *lecture)
{
    int test = 0;
    int i=0;
...
/* Initialisation du module Kineis */
int initModuleKineis()
{
    ...
    //if((tty_fd = open(MODEM , O_RDWR )) == -1){ /* Ouverture en lecture bloquante : il faudrait gérer un timeout avec un signal sur ALARM */
        ...
        perror("ilg_kineis"); //Just if you want user interface error control
        return -1;
    }
    else
    {
